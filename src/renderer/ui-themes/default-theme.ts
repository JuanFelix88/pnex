...
export class DefaultTheme extends ThemeBase {
  ...
  public override async render(ctx: ThemeContext) {
    ...
    const hasRepository = await ctx.isFile(gitConfigPath);

    await new Promise((res) => setTimeout(res, 100)); // Simulate loading time for better skeleton effect
    ...
    if (hasRepository) {
      ...
      // git branch:
      branchRepoEl.innerHTML = `<span>(</span>${svgBranch}<span>${branchName})</span>`;
      branchRepoEl.classList.remove("default-theme-skeleton");
      branchRepoEl.style.width = "";

      // git actual user:
      repositoryUsernameEl.innerText = repositoryUsername ?? "";
      repositoryUsernameEl.innerText = `[${repositoryUsernameEl.innerText}]`;
      repositoryUsernameEl.classList.remove("default-theme-skeleton");
      repositoryUsernameEl.style.width = "";
      if (!repositoryUsername) repositoryUsernameEl.remove();
    } else {
      branchRepoEl.remove();
