...
const svgAI = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path fill="currentColor" fill-rule="evenodd" d="M16.404,5.561l1.823,3.333	c0.184,0.336,0.459,0.611,0.795,0.795l3.333,1.823c0.748,0.409,0.748,1.483,0,1.892l-3.333,1.823	c-0.336,0.184-0.611,0.459-0.795,0.795l-1.823,3.333c-0.409,0.748-1.483,0.748-1.892,0l-1.823-3.333	c-0.184-0.336-0.459-0.611-0.795-0.795l-3.333-1.823c-0.748-0.409-0.748-1.483,0-1.892l3.333-1.823	c0.336-0.184,0.611-0.459,0.795-0.795l1.823-3.333C14.921,4.813,15.995,4.813,16.404,5.561z" clip-rule="evenodd" opacity=".35"/><path fill="currentColor" fill-rule="evenodd" d="M6.82,2.539l0.656,1.519c0.09,0.209,0.257,0.376,0.466,0.466L9.461,5.18	c0.719,0.31,0.719,1.33,0,1.641L7.942,7.476c-0.209,0.09-0.376,0.257-0.466,0.466L6.82,9.461c-0.31,0.719-1.33,0.719-1.641,0	L4.524,7.942c-0.09-0.209-0.257-0.376-0.466-0.466L2.539,6.82c-0.719-0.31-0.719-1.33,0-1.641l1.519-0.656	c0.209-0.09,0.376-0.257,0.466-0.466L5.18,2.539C5.49,1.82,6.51,1.82,6.82,2.539z" clip-rule="evenodd"/><path fill="currentColor" fill-rule="evenodd" d="M4.615,15.404l0.492,1.139c0.068,0.157,0.193,0.282,0.35,0.35l1.139,0.492	c0.539,0.233,0.539,0.998,0,1.23l-1.139,0.492c-0.157,0.068-0.282,0.193-0.35,0.35l-0.492,1.139c-0.233,0.539-0.998,0.539-1.23,0	l-0.492-1.139c-0.068-0.157-0.193-0.282-0.35-0.35l-1.139-0.492c-0.539-0.233-0.539-0.998,0-1.23l1.139-0.492	c0.157-0.068,0.282-0.193,0.35-0.35l0.492-1.139C3.618,14.865,4.382,14.865,4.615,15.404z" clip-rule="evenodd"/></svg>`;
...
export class DefaultCommandTheme extends ThemeCommandBase {
  ...
  public override async render(ctx: ThemeContext) {
    ctx.clearUi();
    const isTrashCommandHistory =
      (this.status === "error" || this.status === "success") && !this.isLoaded;
    ...
    if (!this.isChatMode) {
      ...
      statusDotEl.classList.add("default-theme-status-dot");
      statusDotEl.innerHTML = "•";
      statusDotEl.dataset.status = this.status;
      ctx.elementContainer.appendChild(statusDotEl);
    } else {
      const statusAiEl = document.createElement("div");
      statusAiEl.classList.add("default-theme-status-ai");
      statusAiEl.innerHTML = svgAI;
      statusAiEl.dataset.status = this.status;
      ctx.elementContainer.appendChild(statusAiEl);
    }
    ...
    !isTrashCommandHistory && ctx.elementContainer.appendChild(branchRepoEl);
